...
export default function CalmPlan() {
  ...
  const totalEntries = useCalmPlanStore((s) => s.entries.length);
  const [expanded, setExpanded] = useState(false);
  const showBuilder = expanded || totalEntries > 0;
  ...
  return (
    <Screen scroll overlay={<FloatingHelpButton />}>
      ...
      {!showBuilder && (
        <EmptyState
          glyph="⛰"
          message="Build this while you feel okay, so it can support you when you don't."
          actionLabel="Create my calm plan"
